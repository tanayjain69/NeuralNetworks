# Learnt how to build a basic image generation model (GAN)
# Tried using Residual Connection ideas to it
# Playing with hyperparameters to find the prefect fit. (Even trying ratio in every epoch)
...
disc_lr = 0.00015
gen_lr = 0.00015
RATIO = 1   #Training per epoch (discriminator=1, generator=RATIO)
...
if __name__ == '__main__':
    ...
    # disc.load_state_dict(torch.load('./ImageGeneration/models/disc-res128.pth', weights_only=True))
    disc.to(DEVICE)

    gen = Generator()
    gen_loss = nn.BCEWithLogitsLoss()
    gen_optimiser = torch.optim.Adam(gen.parameters(), lr=gen_lr, betas=(0.5, 0.999))
    # gen.load_state_dict(torch.load('./ImageGeneration/models/gen-res128.pth', weights_only=True))
    gen.to(DEVICE)
    ...
    for epoch in range(EPOCHS):
        ...
        torch.save(disc.state_dict(), './ImageGeneration/models/disc-res64.pth')
        torch.save(gen.state_dict(), './ImageGeneration/models/gen-res64.pth')
